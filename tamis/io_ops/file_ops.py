...
from tamis.models.library import ImageLibrary
...
def rename_with_sequence(item: ImageItem, name: str, number: int) -> Path:
    ...
    # *other* files.
    temp = unique_path(item.path.with_name(f".tamis_rename_{uuid.uuid4().hex}{item.path.suffix}"))
...
def renumber_by_creation_time(items: list[ImageItem], name: str) -> RenumberReport:
    ...
    `.tamis_renumber_<uuid><ext>` name rather than restored to its original
    ...
    for item in ordered:
        temp = unique_path(item.path.with_name(f".tamis_renumber_{uuid.uuid4().hex}{item.path.suffix}"))
        try:
            item.path.rename(temp)
...
def rename_by_creation_date(items: list[ImageItem]) -> RenumberReport:
    ...
    `.tamis_rename_<uuid><ext>` name rather than restored automatically.
